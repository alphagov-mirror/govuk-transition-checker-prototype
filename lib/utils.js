...
exports.addNunjucksFilters = function (env) {
  ...
  const individualFiltersFolder = path.join(__dirname, '../app/filters')

  if (fs.existsSync(individualFiltersFolder)) {
    var files = fs.readdirSync(individualFiltersFolder)
    files.forEach(file => {
      fileData = require(path.join(individualFiltersFolder, file))
      ...
      Object.keys(fileData).forEach((filterGroup) => {
        ...
        Object.keys(fileData[filterGroup]).forEach(filterName => {
          individualFilters[filterName] = fileData[filterGroup][filterName]
        })
      })
    })
  }

  var filters = Object.assign(coreFilters, individualFilters, customFilters)
