...
(function (Modules) {
  ...
  Modules.Feedback = function () {
    this.start = function ($element) {
      ...
      this.$fields = $element.find('.govuk-pub-feedback__form-field')
      ...
      function setHiddenValuesNotUsefulForm (gaClientId) {
        ...
        if (document.querySelectorAll('.govuk-pub-feedback__email-link#take-survey').length === 0) {
          that.$surveyWrapper.append('<a href="https://www.smartsurvey.co.uk/s/gov-uk-banner/?c=' + finalPathName + '&amp;gcl=' + gaClientId + '" class="govuk-pub-feedback__email-link govuk-link" id="take-survey" target="_blank" rel="noopener noreferrer">Don’t have an email address?</a>')
        }
      }
      ...
      function toggleForm (formId) {
        ...
        if (formIsVisible) {
          that.$activeForm.find('.govuk-pub-input').first().focus()
        } else {
      ...
      function showError (error) {
        var genericError = [
          ...
          '</h2>',
          '<p class="govuk-body">If the problem persists, we have other ways for you to provide',
          ' feedback on the <a class="govuk-link" href="/contact/govuk">contact page</a>.</p>'
        ].join('')
